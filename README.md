...
The application is responsive web app that support mobile, tablet and dektop resolution. Application is server side rendered and the setup instructions are described under the following contents.
...
* npm
If you do not have latest version of npm use the following commang to update
...
```sh
git clone https://github.com/jintzz/SpaceX-Launch-programs-Demo.git
```
...
This project helps user to view the SpaceX mission details and filter the results based on the Launch Year, Launch status and landing status.
 On hitting the base url (http://localhost:4000/) user will be automatically redirected to the landing page (http://localhost:4000/launchBoard), where the filter and the mission details are listed.
 <br/>
...
```
Bulding and serving the application using Express server makes it SSR. Unlike normal client side rendered applications that loads in port :4200, SSR applications are loaded in port:4000.
<br/>
...
### App Component: Basic component that contain the primary layout
### App routing module: Handle the routes in the application
### App server module: Enable SSR implementation and handle routes
### Filter component: Handle filter operations. Filters are categorised based on Year, Landing and Launch status. Values are hardcoded. The URL will be updated based on filter selections and prevent page from redirection.
### Launch List: Allows dynamic content rendering. Mission list from API response are mapped and manipulated and rendered in UI with responsve design
### Landing Page: Contains the layout for the content area. It contains the filter component and launch list and manages the allignment as per screen size
### Launch Manager Service: Handle API request made to the external URL for fetching the details, depending on the selection. Response is emmited with the help of BehaviorSubject from 'Rxjs' so that components subscribing to the request will get updated as soon as the response is received.
<br/>
...
Project Link: [https://github.com/jintzz/SpaceX-Launch-programs-Demo.git](https://github.com/jintzz/SpaceX-Launch-programs-Demo.git)
